  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        {loading ? <div className="loading">Loading workspace…</div> : <>{tab==="dashboard"&&<Dashboard data={data}/>} {tab==="senders"&&<Senders data={data} refresh={refresh}/>} {tab==="prospects"&&<Prospects data={data} refresh={refresh}/>} {tab==="campaigns"&&<Campaigns data={data} refresh={refresh}/>}</>}
      </main>
    </div>
  );
}
...
function Auth({mode,setMode,email,setEmail,password,setPassword,error,setError}) {
  const submit = async e => { e.preventDefault(); setError(""); const fn = mode==="login" ? supabase.auth.signInWithPassword({email,password}) : supabase.auth.signUp({email,password}); const { error } = await fn; if(error) setError(error.message); };
...
function Dashboard({data}) { const totalSent=data.senders.reduce((a,s)=>a+s.sent_today,0); const capacity=data.senders.reduce((a,s)=>a+Math.max(0,s.daily_limit-s.sent_today),0); return <section><div className="stats"><Stat label="Connected Gmail" value={data.senders.length}/><Stat label="Sent today" value={totalSent}/><Stat label="Capacity remaining" value={capacity}/><Stat label="Campaigns" value={data.campaigns.length}/></div><div className="grid2"><Panel title="Sender pool"><p className="muted">Eligible accounts rotate in order. Exhausted or disconnected accounts are skipped.</p>{data.senders.slice(0,8).map(s=><div className="row" key={s.id}><span>{s.email}</span><span>{s.sent_today}/{s.daily_limit}</span></div>)}</Panel><Panel title="Campaigns"><p className="muted">Create one campaign with multiple message variants.</p>{data.campaigns.slice(0,6).map(c=><div className="row" key={c.id}><span>{c.name}</span><span className={`pill ${c.status}`}>{c.status}</span></div>)}</Panel></div></section>; }
