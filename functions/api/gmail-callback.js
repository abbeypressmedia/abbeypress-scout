...
export async function onRequestGet({request,env}){
  try{
    ...
    const profile=await fetch("https://gmail.googleapis.com/gmail/v1/users/me/profile",{headers:{Authorization:`Bearer ${tokens.access_token}`}}).then(r=>r.json());
    if(!profile.emailAddress) throw new Error("Could not read Gmail profile");
    const sb=createClient(env.SUPABASE_URL,env.SUPABASE_SERVICE_ROLE_KEY);
    const {error}=await sb.from("sender_accounts").upsert({
      user_id:uid,email:profile.emailAddress,google_subject:profile.emailAddress,
      refresh_token:tokens.refresh_token,status:"connected",updated_at:new Date().toISOString()
  ...
  }catch(e){return html(400,`<p>Connection failed: ${String(e.message).replace(/[<>&]/g,m=>({"<":"&lt;",">":"&gt;","&":"&amp;"}[m]))}</p>`);}
}