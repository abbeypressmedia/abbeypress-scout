...
const raw=(to,subject,body)=>btoa(unescape(encodeURIComponent(
...
const isAuthFailure=e=>e?.status===401||e?.code==="invalid_grant"||/invalid_grant|invalid authentication|invalid credentials/i.test(e?.message||"");
const isRetryable=e=>[429,500,502,503,504].includes(e?.status);
...
export async function processCampaign(sb,env,campaignId,userId){
  ...
  try{
    ...
    const finalized=await finalize(sb,{
      p_claim_token:job.claim_token,p_user_id:userId,p_sender_id:job.sender_id,p_message_id:job.message_id,
      p_status:"failed",p_provider_message_id:null,p_error:`${job.sender_email}: ${e.message}`,
      p_next_send_at:new Date(Date.now()+retrySeconds*1000).toISOString(),
      p_permanent:permanent,p_auth_failure:authFailure
    });
